...
simulate_slugs_and_soybeans <- function(n, min_slugs_observed, max_slugs_observed, 
                                        intercept, effect_of_slug_increase, sd,
                                        slug_interval, n_reps){
  
  if(n_reps > 1){
    
    intercepts <- vector(length = n_reps)
    slopes <- vector(length = n_reps)
    pred_array <- array(data = NA, dim = c(15, 3, n_reps)) # 15 is the newdata divison for the plots
    ...
    for(i in 1:n_reps){
      ...
      pred_array[1:nrow(newdata),1:3,i] <- as.matrix(predict(fit1, newdata, interval = 'confidence'))
    }
    ...
    lines(pred_mean[,1] ~ newdata$slugs_per_trap, col = 'black', lwd = 2)
    # plot the lower and upper 95% cI for the mean response for a given number of slugs per trap
    lines(pred_mean[,2] ~ newdata$slugs_per_trap, col = 'blue', lty = 2, lwd = 2)
    lines(pred_mean[,3] ~ newdata$slugs_per_trap, col = 'blue', lty = 2, lwd = 2)
    ...
    R2 <- summary(fit1)$r.squared
    # intercept term
    intercept <- summary(fit1)$coefficients[1,1]
    # effect of slug increase
